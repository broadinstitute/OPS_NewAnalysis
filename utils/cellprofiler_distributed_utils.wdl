...
task cellprofiler_pipeline_task {
  ...
  String barcode_filename = "Barcodes.csv"
  ...
  command <<<
    ...
    # copy barcode csv into the Default Input Folder used by CellProfiler (-i)
    cp ~{barcode_csv} "$csv_dir/~{barcode_filename}"
    ...
    echo "Barcode CSV copied into Default Input Folder ==================="
    ls -lah "$csv_dir"
